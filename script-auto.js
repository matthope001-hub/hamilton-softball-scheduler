// Enhanced March Madness Bracket Tracker 2026 with Automatic Results
class MarchMadnessBracket {
    ...
    checkForScheduledUpdates() {
        // Check if it's March 2026 and tournament is active
        const now = new Date();
        const tournamentStart = new Date('2026-03-15');
        const tournamentEnd = new Date('2026-04-07');
        
        if (now >= tournamentStart && now <= tournamentEnd) {
            // Auto-enable updates during tournament
            setTimeout(() => {
    ...
    enableAutoUpdate() {
        ...
        // Start fetching results every 5 minutes
    ...
    async fetchTournamentResults() {
        try {
            ...
            if (results) {
                ...
                this.updateLeaderboard();
                this.updateTournamentResults();
                this.showNotification('Tournament results updated!');
            }
        } catch (error) {
    ...
    async getRealTournamentResults() {
        ...
        // Return current tournament results (simulated)
    ...
    simulateFullTournament() {
        if (confirm('Simulate entire tournament? This will generate random results for all rounds.')) {
            ...
            this.updateLeaderboard();
            this.updateTournamentResults();
            this.showNotification('Tournament simulation complete!');
        }
    }

    generateTournamentResults() {
        // Simulate realistic tournament results
        const upsets = [0.15, 0.2, 0.25, 0.3, 0.35, 0.4]; // Increasing upset probability by round
        ...
        for (let round = 2; round <= 6; round++) {
            ...
            // Set teams for current round based on previous round winners
